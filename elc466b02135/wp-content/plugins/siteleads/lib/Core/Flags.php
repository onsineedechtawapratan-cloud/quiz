...
class Flags {
	...
	private function __construct() {
		...
		$this->flags = get_option( '__siteleads_instance_flags', array() );
		add_action( 'shutdown', array( $this, 'save' ) );

		$default_settings = require_once SITELEADS_ROOT_DIR . '/defaults/flags.php';
	...
	private function withFlags( $action, $flag = null, $data = null ) {
		...
		if ( $action === 'get' ) {
			...
			$flags_defaults = apply_filters( 'siteleads/instance-flags-default', array() );

			if ( isset( $flags_defaults[ $flag ] ) ) {
				return $flags_defaults[ $flag ];
			}
	...
	public function save() {
		if ( $this->is_dirty_value ) {
			update_option( '__siteleads_instance_flags', $this->flags, false );
	...
	public static function getSettings( $raw = false ) {
		...
		return apply_filters( 'siteleads/flags/settings', $settings );
	}
