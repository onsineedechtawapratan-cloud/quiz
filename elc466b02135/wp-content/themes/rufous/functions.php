...
function rufous_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'rufous_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function rufous_theme() {
	return Theme::getInstance();
}

function rufous_assets() {
	return rufous_theme()->getAssetsManager();
}
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	rufous_assets()
		...
		->registerStylesheet( 'rufous-theme', '/theme/theme.css' );
	rufous_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		rufous_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'rufous_print_color_scheme', 0 );
} else {
	rufous_assets()
		->registerStyle( 'rufous-theme', rufous_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function rufous_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'rufous_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'rufous_after_activation_redirect_url' );
function rufous_after_activation_redirect_url( $url ) {
...
function rufous_get_builder_plugin_slug() {
...
function rufous_theme_plugins( $plugins ) {

	$plugin_slug = rufous_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'rufous' ) : __( 'Kubio', 'rufous' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'rufous_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === rufous_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported kubio-notice notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'rufous' ),
									rufous_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'rufous' ),
									rufous_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'rufous' ) ); ?>
                        </a>
...
add_filter( 'kubio/editor-try-online/url', 'rufous_try_online_url', 10 );

function rufous_render_header_style() {
